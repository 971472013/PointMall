<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="商品类型" width="130px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column fixed="right" label="操作" align="center" width="153px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-row>
            <el-button type="primary" size="mini" @click="add2suject(row)">
              添加到专题
            </el-button>
          </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      editRules: {
        subjectName: [{ required: true, message: '专题名称为必选项', trigger: 'blur' }],
        subjectDescribe: [{ required: true, message: '专题描述为必选项', trigger: 'blur' }]
      },
  ...
  created() {
    this.$store.dispatch('subject/getMallGoodsWithSubject', { inSubject: false }).then(() => {
  ...
  methods: {
    ...
    filter() {
      ...
      this.$store.dispatch('subject/filterGoodsInSubject', { inSubject: false, ...this.searchForm }).then(() => {
        this.$notify({
          title: 'Success',
          message: '查询成功',
          type: 'success',
    ...
    add2suject(row) {
      this.$store.dispatch('subject/addGoodsToSubject', { goodsID: row.id }).then(() => {
        this.$notify({
          title: 'Success',
          message: '商品已添加至专题',
          type: 'success',
